...
root.columnconfigure(0, weight=1)  # Без это строчки всё будет липнуть к левому краю
...
ttk.Combobox(r_info, textvariable=variables["Plot"], values=list(range(1, 21))).grid(
    row=3, column=1, sticky=(tk.W + tk.E)
)
variables["Seed Sample"] = tk.StringVar()
ttk.Label(r_info, text="Seed Sample").grid(row=2, column=2)
ttk.Entry(r_info, textvariable=variables["Seed Sample"]).grid(
...
ttk.Spinbox(
    e_info, textvariable=variables["Temperature"], from_=4, to=40, increment=0.01
...
ttk.Spinbox(p_info, textvariable=variables["Plants"], from_=0, to=20, increment=1).grid(
    row=1, column=0, sticky=(tk.W + tk.E)
)
